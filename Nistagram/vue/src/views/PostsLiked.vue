...
<script>
import { myVar } from '../components/Search.vue'
...
export default {
  ...
  data: function () {
       ...
       await  axios
                .get("http://localhost:8080/api/post/getPostsByUserId/" + localStorage.getItem('userId'),
				{
							headers: {
                ...
                .then(response => {
                  if (response.status==200){
                    ...
                    for(let j = 0; j < response.data.length; j++){
                     ...
                     for(let x = 0; x < response.data[j].likes.length; x++){
                         if(response.data[j].likes[x] == localStorage.getItem('username')){
                        ...
                        for (let i = 0; i < response.data[j].images.length; i++) {
                           // pom1 = this.pict[j].images[i].filepath.split('\\')
                            //if (pom1.length == 4) {
                                ...
                                if(response.data[j].images[i].filename.includes('mp4')){
                                    response.data[j].images[i].img = 'data:video/mp4;base64,' + response.data[j].images[i].img
                                    alert(response.data[j].images[i].img)
                                }else{
                                    response.data[j].images[i].img = 'data:image/png;base64,' + response.data[j].images[i].img
                                }
                            //}
                          }
                          this.pict.push(response.data[j])
                          break
                         }
                     }
                      }
